...
function validatePackageJson() {
  ...
  const missingFiles = REQUIRED_FILES.filter(file => 
    !files.some(f => file.startsWith(f) || f.includes(file))
  );
...
function validateExamples() {
  ...
  try {
    ...
    runCommand('node -e "import(\'./examples/async-example.js\').catch(() => process.exit(1))"', 'Async example syntax check');
    runCommand('node -e "import(\'./examples/sync-example.js\').catch(() => process.exit(1))"', 'Sync example syntax check');
    runCommand('node -e "import(\'./examples/testing-example.js\').catch(() => process.exit(1))"', 'Testing example syntax check');
    return true;
  } catch (error) {
