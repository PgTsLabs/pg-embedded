...
async function testConcurrentOperations(testSuite) {
  ...
  try {
    ...
    const existsPromises = dbNames.map(name => 
      testSuite.postgres.databaseExists(name)
    );
    ...
    const cleanupPromises = dbNames.map(name => 
      testSuite.postgres.dropDatabase(name)
    );
...
async function testErrorHandling(testSuite) {
  ...
  try {
    ...
    try {
      ...
      console.log('✅ Correctly caught error for empty database name');
    }
    ...
    try {
      ...
      console.log('✅ Dropping non-existent database threw expected error');
    }
...
async function testPerformance(testSuite) {
  ...
  try {
    ...
    console.assert(avgTime < maxAvgTime, 
      `Average operation time (${avgTime}ms) should be less than ${maxAvgTime}ms`);
